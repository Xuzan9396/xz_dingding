...
function closeApp(packageName) {
  ...
  sleep(2000);
  // console.log(textMatches(/(.*强.*|.*停.*|.*结束.*)/).findOne());

  let is_sure = textMatches(/(.*强.*|.*停.*|.*结束.*)/).findOne();
  if (is_sure.enabled()) {
      console.log("找到强行停止按钮");
      clickB(is_sure);
      sleep(1000)
      clickB(textMatches(/(.*确.*|.*定.*|.*强行停止*.)/).findOne());
      log(app.getAppName(name) + "应用已被关闭");
      sleep(1000);
      back();
  } else {
